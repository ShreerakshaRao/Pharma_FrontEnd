...
export const createPurchase = async (formData: PurchaseData): Promise<PurchaseResponse> => {
    try {
      const response = await api.post<{ data: PurchaseResponse; message: string; status: string }>(
        'pharma/stock/save',
        formData
      );
...
// Get PurchaseById
export const getPurchaseById = async (invId: number) => {
  try {
