...
export const createSupplier = async (formData: Supplier1): Promise<Supplier1> => {
    try {
      const response = await api.post<{ data: Supplier1; message: string; status: string }>(
...
export const updateSupplier = async (supplierId: number, supplierData: Supplier1) => {
    try {
